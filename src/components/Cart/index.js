...
const Cart = () => {
    ...
    const handleInc=(id)=>{
        const upadatedCart= carts.map(item =>{
            if(item.id === id){
                return {
                    ...item,
  ...
  if (carts.length ===0){ return<div className='h-[55vh] flex justify-center items-center text-4xl'>Cart is Empty</div>}
        ...
        </div>
        {
          ...
          <div class="flex w-2/5"> 
            ...
            <div class="flex flex-col justify-between ml-4 flex-grow">
              ...
              <a href="#" class="font-semibold hover:text-red-500 text-gray-500 text-xs" onClick={()=>productRemove(cart?.id)}>Remove</a>
            </div>
          </div>
          <div class="flex justify-center w-1/5">
            <svg class="fill-current text-gray-600 w-3 cursor-pointer" viewBox="0 0 448 512" onClick={()=> handleDec(cart?.id)}><path d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z"/>
            </svg>

            <input class="mx-2 border text-center w-8" type="text" value={cart?.quantity}/>

            <svg class="fill-current text-gray-600 w-3 cursor-pointer" viewBox="0 0 448 512" onClick={()=> handleInc(cart?.id)}>
